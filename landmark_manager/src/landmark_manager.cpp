/// \file
/// \brief This node provides a service for saving landmarks, which are displayed as markers.
/// It also provides services for canceling navigation and navigating to landmarks.
///
/// PUBLISHES:
///     /landmark_markers (visualization_msgs::msg::MarkerArray): sphere markers that represent
///                                                               landmarks as well as text markers
/// SERVERS:
///     save_landmark (landmark_manager::srv::SaveLandmark): saves the landmark name and position
///     navigate_to_landmark (landmark_manager::srv::NavigateToLandmark): sends a specific landmark
///                                                                       as a goal
///     cancel_navigation (std_srvs::srv::SetBool): cancels the navigation
/// CLIENTS:
///     navigate_to_pose (nav2_msgs::action::NavigateToPose): navigates from a starting point to a
///                                                           goal
...
/// \brief Provides functionalities for saving landmarks, canceling navigation, and navigating to
/// landmarks
...
public:
  ...
  : Node("landmark_manager")
  {
    ...
    save_service_ = create_service<landmark_manager::srv::SaveLandmark>(
      "save_landmark",
      std::bind(
        &LandmarkManager::save_landmark_callback, this,
        std::placeholders::_1, std::placeholders::_2));
    navigate_service_ = create_service<landmark_manager::srv::NavigateToLandmark>(
      "navigate_to_landmark",
      std::bind(
        &LandmarkManager::navigate_to_landmark_callback, this,
        std::placeholders::_1, std::placeholders::_2));
    cancel_service_ = create_service<std_srvs::srv::SetBool>(
      "cancel_navigation",
      std::bind(
        &LandmarkManager::cancel_navigation_callback, this,
        std::placeholders::_1, std::placeholders::_2));
...
private:
  ...
  /// \brief Callback function for the save_landmark service. Saves a new landmark.
  ///
  ...
  /// \brief Callback function for the navigate_to_landmark service. Navigates to a specific
  /// landmark.
  ///
    ...
    landmark_manager::srv::NavigateToLandmark::Response::SharedPtr /*response*/)
  {
    ...
    if (!file_exists(file_path)) {
      RCLCPP_WARN(rclcpp::get_logger("LandmarkManager"), "No landmarks file found");
      return;
    }
    ...
    result)
  {
    switch (result.code) {
      case rclcpp_action::ResultCode::SUCCEEDED:
        RCLCPP_INFO(rclcpp::get_logger("LandmarkManager"), "Navigation succeeded");
        break;
      case rclcpp_action::ResultCode::ABORTED:
        RCLCPP_ERROR(rclcpp::get_logger("LandmarkManager"), "Navigation failed");
        break;
      case rclcpp_action::ResultCode::CANCELED:
        RCLCPP_INFO(rclcpp::get_logger("LandmarkManager"), "Navigation was canceled");
        navigation_canceled_ = true;
        break;
      default:
        RCLCPP_ERROR(rclcpp::get_logger("LandmarkManager"), "Unknown result code");
        break;
    }
  }

  /// \brief Callback function for the cancel_navigation service. Cancels an ongoing navigation.
  ///
  /// \param request - not being used
  /// \param response - response indicating whether the cancellation was successful
  /// \return none
    ...
    std_srvs::srv::SetBool::Response::SharedPtr response)
  {
    ...
    if (!action_client_ || !action_client_->action_server_is_ready()) {
      RCLCPP_WARN(rclcpp::get_logger("LandmarkManager"), "Action server not available");
  ...
  {
    std::string package_path = ament_index_cpp::get_package_share_directory("landmark_manager");
  ...
  rclcpp::Service<landmark_manager::srv::NavigateToLandmark>::SharedPtr navigate_service_;
